...
impl World {
    ...
    pub fn get_singleton_mut<'a, T: Component>(&'a mut self) -> Option<&'a mut T> {
        ...
        let dest =
            unsafe { ecs_get_mut_id(self.world, entity.raw(), comp_id.raw(), &mut is_added) };
    ...
    pub fn each_3<'a, A: Component, B: Component, C: Component>(
        &'a self,
        mut cb: impl FnMut(Entity, &A, &B, &C),
    ) {
    ...
    // Iterate through all entities matching 2 components
    pub fn each_mut_2<'a, A: Component, B: Component>(
        &'a self,
        mut cb: impl FnMut(Entity, &mut A, &mut B),
    ) {
        let filter: FilterGroup<'a, (A, B)> = FilterGroup::new(self);
    ...
    // Iterate through all entities matching 3 components
    pub fn each_mut_3<'a, A: Component, B: Component, C: Component>(
        &'a self,
        mut cb: impl FnMut(Entity, &mut A, &mut B, &mut C),
    ) {
        let filter: FilterGroup<'a, (A, B, C)> = FilterGroup::new(self);
